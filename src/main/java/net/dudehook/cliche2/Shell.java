...
/*
 * Introducing the asg.cliche (http://cliche.sourceforge.net/) Cliche is to be a VERY simple reflection-based command
 * line shell to provide simple CLI for simple applications. The name formed as follows: "CLI Shell" --> "CLIShe" -->
 * "Cliche".
 */
...
import net.dudehook.cliche2.util.ArrayHashMultiMap;
import net.dudehook.cliche2.util.MultiMap;
...
public class Shell
{
    ...
    /**
     * Method for registering command hanlers (or providers?) You call it, and from then the Shell has all commands
     * declare in the handler object.
     * 
     * This method recognizes if it is passed ShellDependent or ShellManageable and calls corresponding methods, as
     ...
     * @param prefix
     *            Prefix that should be prepended to all handler's command names.
     */
    ...
    /**
     * This method is very similar to addMainHandler, except ShellFactory will pass all handlers registered with this
     * method to all this shell's subshells.
     * 
    ...
    /**
     * Runs the command session. Create the Shell, then run this method to listen to the user, and the Shell will invoke
     * Handler's methods.
     * 
     ...
     * @param line
     *            Full command line
     * @throws asg.cliche.CLIException
    ...
    protected void processCommand(String discriminator, List<Token> tokens) throws CLIException
    {
        ...
        Object[] parameters = inputConverter.convertToParameters(tokens, paramClasses, commandToInvoke.getMethod().isVarArgs());

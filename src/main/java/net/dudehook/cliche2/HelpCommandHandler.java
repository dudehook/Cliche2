...
public class HelpCommandHandler implements ShellDependent
{
    ...
    public String generateHTMLHelp(@Param(name = "file-name", description = "Path to the file to save the table to.") String fileName,
            @Param(name = "include-prefixed", description = "Whether to include commands with prefix "
                    + "(usually system or advanced functionality).") boolean includePrefixed) throws IOException
    {

        final String HTML_FORMAT = "<html><head><title>Auto-generated command reference file</title></head>" + "<body>\n"
                + "<h1>%1$s Command Reference</h1>\n" + "<em>Auto-generated by the <a href=\"" + Shell.PROJECT_HOMEPAGE_URL
                + "\">Cliche Shell</a></em>\n" + "%2$s</body></html>";
    ...
    private static void appendCommandHTML(StringBuilder commandsHTML, ShellCommand command)
    {
        ...
        {
            paramsHTML.append(String.format(PARAM_FORMAT, htmlEncode(ps.getName()), htmlEncode(ps.getValueClass().getSimpleName()), htmlEncode(ps.getDescription())));
        }

        commandsHTML.append(String.format(COMMAND_FORMAT, htmlEncode(command.getPrefix() + command.getAbbreviation()), htmlEncode(command.getPrefix()
                + command.getName()), htmlEncode(formatCommandParamsShort(command)), htmlEncode(command.getDescription()), paramsHTML));
    }
    ...
    public Object help()
    {
        return "This is Cliche shell (" + Shell.PROJECT_HOMEPAGE_URL + ").\n" + "To list all available commands enter ?list or ?list-all, "
                + "the latter will also show you system commands. To get detailed info " + "on a command enter ?help command-name";
    }

    @Command(description = "Show detailed info on all commands with given name")
    public Object help(@Param(name = "command-name", description = "Command name you want help on") String commandName)
    {
    ...
    private static String formatCommandShort(ShellCommand command)
    {
        ...
        return String.format("%s%s\t%s%s\t%s", hasAbbr ? command.getPrefix() : "", hasAbbr ? command.getAbbreviation() : "", command.getPrefix(), command.getName(), formatCommandParamsShort(command));
    }
    ...
    private static String formatCommandLong(ShellCommand command)
    {
        StringBuilder sb = new StringBuilder(
                String.format("Command: %s\n" + "Abbrev:  %s\n" + "Params:  %s\n" + "Description: %s\n", command.getPrefix() + command.getName(), command.getAbbreviation() != null ? command.getPrefix()
                        + command.getAbbreviation()
                        : "(none)", formatCommandParamsShort(command), command.getDescription()));
        if (command.getArity() > 0)
        {
            ...
            if (paramSpecs != null)
            {
                for (int i = 0; i < paramTypes.length; i++)
                {
                    ...
                    {
                        sb.append(String.format("%s\t%s\t%s\n", paramSpecs[i].getName(), paramTypes[i].getSimpleName(), paramSpecs[i].getDescription()));
                    }
                }
            }
            if (command.getMethod().isVarArgs())
